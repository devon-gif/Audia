...
// 1. REPAIR SUMMARIZE API (Fixed Template Literals)
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        { 
          role: "system", 
          content: "You are a professional narrator. Create a narrative 'Deep Signal Brief'. Avoid bullets. Target " + wordTarget + " words." 
        },
...
// 2. REPAIR GLOBAL PLAYER (Clean Hook Order)
    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-full max-w-2xl px-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="relative w-full bg-zinc-900/90 backdrop-blur-2xl border border-white/10 rounded-2xl p-4 shadow-2xl">
        <div className="flex items-center gap-4">
          ...
          <button onClick={dismiss} className="p-2 text-zinc-500 hover:text-white"><X size={16} /></button>
        </div>
...
console.log('✅ Audia repair complete. All syntax errors purged!');
